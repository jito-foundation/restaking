...
async fn test_initialize_multiple_operators_ok() {
    ...
    let operator_pubkey1 =
        Operator::find_program_address(&jito_restaking_program::id(), &operator_base1.pubkey()).0;
    ...
    let operator_pubkey2 =
        Operator::find_program_address(&jito_restaking_program::id(), &operator_base2.pubkey()).0;
