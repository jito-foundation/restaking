...
impl Operator {
    ...
    ///   returns a `RestakingError::OperatorAdminInvalid` error if the admin account does not match the expected value.
    ///
    /// # Errors
    /// This function will return a `RestakingError::OperatorAdminInvalid` error in the following case:
